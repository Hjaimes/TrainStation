...
if "%TORCH_CHOICE%"=="1" (
    ...
    if %errorlevel% neq 0 (
        echo   %YELLOW%WARNING: torch not found — you may need to install it manually.%RESET%
    )
) else (
    echo %RED%Invalid choice. Defaulting to CUDA 12.8.%RESET%
    pip install torch torchvision --index-url https://download.pytorch.org/whl/cu128
)
...
if /i "%OPT_CHOICE%"=="y" (
    ...
    if %errorlevel% neq 0 (
        echo   %YELLOW%WARNING: Some optional optimizers failed to install. This is OK.%RESET%
    ) else (
