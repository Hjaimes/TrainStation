...
# ---------------------------------------------------------------------------
# Inlined from musubi_tuner/utils/model_utils.py: create_cpu_offloading_wrapper
# ---------------------------------------------------------------------------
...
def create_cpu_offloading_wrapper(func: Callable, device: torch.device) -> Callable:
    """
    Create a wrapper function that offloads inputs to CPU before calling the
    original function and moves outputs back to the specified device.

    Inlined from musubi_tuner/utils/model_utils.py.
    """
...
# Cache for sinusoidal frequency vectors keyed by (dim, device)
_sinusoidal_freq_cache: dict[tuple[int, torch.device], torch.Tensor] = {}
...
def sinusoidal_embedding_1d(dim, position):
    ...
    # Cache the frequency vector (only depends on dim, not position)
    cache_key = (dim, position.device)
    if cache_key not in _sinusoidal_freq_cache:
        _sinusoidal_freq_cache[cache_key] = torch.pow(
            10000, -torch.arange(half, device=position.device, dtype=torch.float64).div(half)
        )
    freqs = _sinusoidal_freq_cache[cache_key]
...
def rope_apply_inplace_cached(x, grid_sizes, freqs_list):
    """Inplace version of rope_apply using pre-cached frequency tensors."""
...
class WanModel(nn.Module):  # ModelMixin, ConfigMixin):
    ...
    def enable_gradient_checkpointing(self, activation_cpu_offloading=False):
        ...
        logger.info("WanModel: Gradient checkpointing enabled. Activation CPU offloading: %s", activation_cpu_offloading)
    ...
    def enable_block_swap(self, blocks_to_swap: int, device: torch.device, supports_backward: bool, use_pinned_memory: bool = False):
        ...
        logger.info(
            "WanModel: Block swap enabled. Swapping %d blocks out of %d blocks. Supports backward: %s",
            self.blocks_to_swap, self.num_blocks, supports_backward,
        )
    ...
    def forward(self, x, t, context, seq_len, clip_fea=None, y=None, skip_block_indices=None, f_indices=None):
        ...
        # Pre-allocate padded buffer instead of nested torch.cat per sample
        bsz = len(x)
        hidden_dim = x[0].size(2)
        padded = x[0].new_zeros(bsz, seq_len, hidden_dim)
        for i, u in enumerate(x):
            padded[i, :u.size(1)] = u[0]
        x = padded
        ...
        skip_set = set(skip_block_indices) if skip_block_indices is not None else set()
        for block_idx, block in enumerate(self.blocks):
            is_block_skipped = block_idx in skip_set
